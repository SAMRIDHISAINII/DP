...
int CountSubsetsWithSum(int arr[], int n, int sum) {
	...
	for (int i = 1; i <= n; i++) {
      ...
      // either exclude or inxlude and add both of them to get final count 
			else
				t[i][j] = t[i - 1][j]; 
      // exclude when element in the list is greater then the sum 
		}
	}

	return t[n][sum]; 
  // finally return the last row and last column element 
}
...
int CountSubsetsWithDiff(int arr[], int n, int diff) {
  ...
  // we will get the number of array(subset) with particular sum 
}
